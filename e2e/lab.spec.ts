...
test.describe('lab and blog content', () => {
  test('lab listing filters content and opens an article plus its tag page', async ({
    page,
  }) => {
    await page.goto('/lab?q=article')

    await expect(
      page.getByRole('heading', { level: 1, name: /articles & experiments/i }),
    ).toBeVisible()
    ...
    await expect(
      page.getByRole('heading', { name: /posts about/i }),
    ).toBeVisible()
  })

  test('legacy blog routes redirect to the TanStack lab routes', async ({
    page,
  }) => {
