...
Create a new Phoenix project using. We only need the bare miniumum to get started, so we'll not add Ecto, Live View or Mailer.

```bash
mix phx.new phoenix_cursors --no-ecto --no-dashboard --no-live --no-mailer
```

I'll be using Tailwind CSS for styling and you can quickly add that to the project using the official installer. You can read about the steps to follow in [this earlier tutorial](https://koenvangilst.nl/blog/adding-tailwind-to-phoenix). All the changes I made to get Tailwind to work are also in [this commit](https://github.com/vnglst/phoenix_cursors/commit/9800403ad1e0b33f30e666957245218589a9e211).

Let's add a nice background and some basic HTML. Replace the contents of the template `lib/phoenix_cursors_web/templates/page/index.html.heex` with the following:
...
First thing we need to do is add a channel that we can use to send mouse position updates to others:

```bash
mix phx.gen.channel Cursor
```

Phoenix added some files to our repo. Make sure to follow any instructions that this commands gives you.

To get a feeling for working with this channel (and sending updates to it)
we'll start by logging cursor movements server side. First we need to start sending events from the client. In
the file `assets/js/user_socket.js` we'll add the following:
...
This listens to changes in the mouse cursor position and sends an even to the Phoenix channel.
...
To display them we need to scale them to the viewport (i.e. multiply them again).
...
If you start the Phoenix server and move around with the mouse,
you'll see the cursor position updates in the console. Yeah, it works!

Now let's also display those updates on the page. For this we'll need a bit of JavaScript.
...
And we'll listen to any 'move' events on the channel:
...
If you open up to browser windows next to each other you can see the cursor moving around. However, the moment you
leave the window the cursor will disappear. To fix this we'll need a way to keep track if users are still present
in the channel. For this we'll need another feature that Phoenix offers, the `Presence` feature.

## Adding names

Before we can track who's online, we'll need a way to identify users. Normally you would use some kind of authentication
(using signup/sign in) and a database for that, but I want to keep this tutorial simple. So instead we'll assign each user
an unique name the moment they join the channel.

To generate unique names we'll use the following function. I'm completely new to Elixir so there's probably a more elegant
way to do this. Please let me know if you know a better way!
...
We'll generate this name on the server and send a signed version to the client. We sign it
to make sure user can't temper with it on the client. This should prevent users from
changing their name to someone else's.
...
defmodule PhoenixCursorsWeb.PageController do
  ...
  def index(conn, _params) do
    render(conn, "index.html",
      user_token: Phoenix.Token.sign(CursorsWeb.Endpoint, "user socket", Cursor.Names.generate())
    )
  end
end
```
...
When a user connects to the socket we can verify its authenticity by checking the token. Once
we've verified that the token is valid, we read the user name from it and assign that name
to the socker connection.

This is done server side in the file `user_socket.ex`:
...
Follow the instructions that are displayed after running this command.

In `user_socket.js` we're going to replace the update logic for the cursors. Instead of rendering user cursors based on the `move` event, we're going to render a list of users that are currently online.
...
After a users joins the channel we'll setup his presence in that channel and when this user send a `move` event, we'll update his Presence state. Update `cursor_channel` to the following:
...
Restart your server and visit `http://localhost:4000/` in your browser. Every user should now have a live cursor.
...
We assign this color the user's presence in `cursor_channel.ex`:
...
And update the function `cursorTemplate` by using the supplied color instead of `pink-300`:
...
What's really cool about Phoenix Presence is that it automatically keeps track of who's online and who's not.
You can verify this if you open up a few browser tabs (to simulate multiple users) and then close any one
of them. That particular user will be removed from the list of online users!

<Disclaimer>
  This concludes the first part of this tutorial. In the next part I'll show you
  how to allow users to send messages using the input field displayed in the
  center. If you followed along in this example you should be able to add this
