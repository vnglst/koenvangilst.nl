...
Create a new Phoenix project using the command line. We only need the bare minimum to get started, so we'll not add Ecto, Live View or Mailer.
...
And we'll listen for 'move' events on the channel:
...
**Disclaimer:** I'm completely new to Elixir so there's probably a more elegant way to do this.
Please let me know if there's better way!
...
After a user joins the channel we setup his presence in that channel. When this user sends a `move` event, we update his Presence state. Update `cursor_channel` to the following:
...
<Disclaimer>
  ...
  how to let users send messages using the input field displayed in the center.
  If you followed along in this example you should be able to add this
