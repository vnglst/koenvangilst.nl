...
In my [previous tutorial](https://koenvangilst.nl/blog/phoenix-live-cursors) I created a feature called "live cursors" using Phoenix Channels and Presence. In this tutorial I'll be recreating the same functionality using Phoenix **LiveView**. When learning some exciting new technology, I generally first want to get a feeling for the problem is tries to solve. That's why I started without LiveView. If you've also followed the previous tutorial you'll discover how much simpler and more readable the implementation becomes when using LiveView.

## What we're building

We'll be building a simple, but compelling software feature I've seen in the design tool Figma. In Figma you can see the mouse cursors of others people working in the document. It's a really cool functionality and Phoenix was made for this!
...
<Waypoint url="https://github.com/vnglst/liveview_cursors/commit/9c02ea98ac6ad6ddabf1b96f4bc17d42afae66e4" />
...
<Waypoint url="https://github.com/vnglst/liveview_cursors/commit/5ceb53e1c94091ec4f07b7f81294b05c8a7d6851" />
...
Showing a copy of your own mouse cursor is nothing special. We want to be able to broadcast this information to every user in the channel and _at the same time_ keep track of who's in that channel. To make that happend we first need to identify our users.

To keep this tutorial simple we'll substitute real authentication with the following: When a user visits the website, a session is created by assigning a random name to a user. After the session is created, we'll redirect to the `/cursors` route. There we can retrieve the user name from the session and display it under to the cursor.

Let's start with a utility function that generates a random name. In `lib/liveview_cursors/names.ex`:
...
Next, update the controller for the root route to create a new session if there is none. After that redirect the user to the `/cursors` route. In `lib/liveview_cursors_web/controllers/page_controller.ex` update the code to the following:
...
Restart the server and open the browser again. You should see the pink cursor again, but this time with a random name assigned to it. This name should persist between page refreshes and when you close the browser (it is stored in a cookie).

<Waypoint url="https://github.com/vnglst/liveview_cursors/commit/0cc3b3578d5366378be369e7e53e46e1a8ceb9c8" />

## Tracking who's online

We'll be using Phoenix Presence to keep track of who's online in the channel. We'll store information about the user (coordinates and name) in the Presence channel. First, add Presence to our application. We'll use the Phoenix generator for doing this:
...
<Waypoint url="https://github.com/vnglst/liveview_cursors/commit/7d70e848b63dfc46c2e2d84e2f81599d626caa8d" />
...
At the end of the `mount` function we're assigning the user, the list of users currently present and the socket id to the LiveView socket. We'll be using that information to render the list of users in the LiveView.
...
We can also handle the `cursor-move` event coming from the client. If we receive this event we'll update the coordinates in the list of users in the Presence channel with the `socket.id` as an identifier. The code for this is as follows:
...
Pfew! Let's see if that worked. Restart the server and open the browser. You should see a pink cursor again. To verify that multiple users are tracked correctly open up another browser window **either in a different browser or in private mode**. We need to make sure that we create a new session. Otherwise you'll just see two you's.

<Waypoint url="https://github.com/vnglst/liveview_cursors/commit/68e580a755d0d8c52257d481e79e3602f8ffd4c2" />
...
<Waypoint url="https://github.com/vnglst/liveview_cursors/commit/5a69d68504d1ce03a975b0b5acf6edb9fe878ec3" />
...
<Waypoint url="https://github.com/vnglst/liveview_cursors/commit/cc65084342e87322a7e52a46632e965dc98c8101" />
...
This concludes my tutorial in which I recreated the live cursors functionality using LiveView. Comparing this approach with my earlier tutorial in which did most of the implementation client side using JavaScript I found that LiveView offers quite a few advantages:
...
- you can update HTML + tailwind classes close to where data lives (colocation of code that frequently changes together)
- the logic for this feature is almost all in the one place (LiveView component)
- this feels familiar to me as a React developer

I hope you enjoyed it and if you have any questions or comments feel free to send me a message on Twitter using the link below.
