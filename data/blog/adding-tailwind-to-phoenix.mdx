...
summary: With the new official mix task it has become very easy to add Tailwind to any Phoenix project. To make it even more straightforward I've written this quick step-by-step guide.
---
...
I'm new to Phoenix myself, so I tried to spell out every step I took. Even things that are (probably) obvious to more experienced Phoenix developers. For people that want to skip to the end, here's a link to the [end result of this tutorial on Github](https://github.com/vnglst/phoenix_tailwind).
...
I've also changed the body styling here to illustrate that the `@apply` functionality also works.

Since Tailwind is now taking care of our CSS, we can comment out the following line in `assets/js/app.js`:
