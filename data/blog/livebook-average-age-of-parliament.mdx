...
In this tutorial I'll go over the steps needed to retrieve, prepare and visualise the data in an Elixir Livebook. At the end of this tutorial you should be able to visualize the average age of politicians of other countries as well.

The corresponding Livebook can be found [here](https://github.com/vnglst/livebook-age-of-dutch-parliament/blob/main/age-of-parliament.livemd).
...
To retrieve the data I used the Wikidata's query engine that is provided free of charge. Wikidata uses the SPARQL query language, which is a powerful language in and by itself. It's somewhat similar to SQL, but it did take me some time to [familiarize myself with the syntax](https://www.wikidata.org/wiki/Wikidata:SPARQL_tutorial).
...
Looking at the plot it seems like the Dutch parliament is getting younger! Pfew, no gerontocracy in my country!
