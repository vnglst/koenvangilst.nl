...
<Disclaimer>
  This tutorial was written in March 2019, when React Hooks were just released.
  It was written in the spirit of those times and when I look back at it now, I
  notice that the tutorial overuses some of the hooks. Nowadays I would be
  hesitant to use `useEffect` so extensively and I would handle side effects in
  event handler functions instead. That said, I still think it's a good tutorial
  to get a feel for how hooks work and how you can compose them to create custom
  hooks.
</Disclaimer>
