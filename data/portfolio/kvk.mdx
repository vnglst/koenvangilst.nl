---
name: Kamer van Koophandel
year: 2018
summary: For the Dutch Chamber of Commerce (KVK), I worked as a frontend engineer in the team responsible for building and deploying the new Online Registration web application. This app enables Dutch entrepreneurs to register their businesses completely online. (In Dutch)
---

## Online Registreren

De Kamer van Koophandel heb ik als frontendontwikkelaar geholpen bij de
ontwikkeling van de applicatie waarmee startende ondernemers (ZZP-ers)
hun nieuwe onderneming online kunnen inschrijven. Met online registreren
sluit KVK aan op de brede maatschappelijke behoefte om transacties (met
overheden) online af te handelen.

Deze applicatie is eind 2019 landelijk live gegaan en wordt jaarlijks
door meer dan 200.000 startende ZZP-ers worden gebruikt. Gebruikers
beoordelen deze applicatie uiterst positief (hoogste tevredenheidsscore
van de KVK).

Sinds juni 2020 is het voor gebruikers ook mogelijk om de registratie
helemaal online af te ronden. Hiervoor wordt gebruikt gemaakt van een
speciale technologie waarmee ook de legitimatie (met een paspoortcheck)
volledig digitaal wordt uitgevoerd. Een bezoek aan een van de
KVK-kantoren is daardoor voor het registreren van een
onderneming niet meer vereist.

<Image
  alt={``}
  src={`/static/screenshots/screenshot-kvk.jpg`}
  width={2490 / 4}
  height={1382 / 4}
/>
