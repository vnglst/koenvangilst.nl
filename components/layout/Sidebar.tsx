    ...
    <aside className="-mx-4 px-4 md:mx-0 md:w-[200px] md:flex-shrink-0 md:px-0">
      <div className="lg:sticky lg:top-20">
        ...
        <nav className="flex flex-row items-center space-x-2 pb-4 md:flex-col md:items-start md:space-x-0 md:px-0 md:pb-0">
          {navItems.map((item) => (
            <div key={item.href} className="flex items-center md:pl-3">
              <div
                className={cx(
                  'mr-2 hidden h-1 w-1 rounded-full transition-opacity md:block',
                  isActive(item.href) ? 'bg-primary opacity-100' : 'opacity-0'
                )}
              />
              <Link
                href={item.href}
                className={cx(
                  'py-1 text-sm whitespace-nowrap transition-opacity',
                  isActive(item.href)
                    ? 'text-primary dark:text-primary font-medium'
                    : 'text-gray-700 hover:opacity-60 dark:text-gray-400'
                )}
              >
                {item.label}
              </Link>
          ...
          <div className="ml-auto flex items-center md:mt-6 md:ml-0 md:pl-3">
            <div className="mr-2 hidden h-1 w-1 opacity-0 md:block" />
            <ThemeToggleText />
          </div>
        </nav>
      </div>
