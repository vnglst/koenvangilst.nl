...
Run the generator manually with `docker compose run --rm og-generator`. For a local non-Docker run, set `OG_OUTPUT_DIR` to a writable directory before running `node scripts/generate-og-images.mjs`.
