...
<Disclaimer>
  Don't forget to also read the [second part of this tutorial](/blog/phoenix-liveview-cursors), where I implement a
  similar functionality using Phoenix LiveView!
</Disclaimer>
...
```html
<section class="flex h-screen w-screen flex-col items-center justify-center text-center">
  <form
    id="msgform"
    class="mx-auto flex w-xs space-x-3 rounded-xl bg-gradient-to-r from-pink-50 to-pink-100 p-8 drop-shadow-xl"
  >
    <input
      class="flex-1 appearance-none rounded-lg border border-transparent bg-white px-4 py-2 text-base text-gray-600 placeholder-gray-400 shadow-md focus:border-transparent focus:ring-2 focus:ring-pink-600 focus:outline-none"
      maxlength="30"
    ...
    <input
      id="submit-msg"
      type="submit"
      class="flex-shrink-0 rounded-lg bg-pink-600 px-4 py-2 text-base font-semibold text-white shadow-md hover:bg-pink-700 focus:ring-2 focus:ring-pink-500 focus:ring-offset-2 focus:ring-offset-pink-200 focus:outline-none"
      value="Change"
    />
  </form>
  <ul id="cursor-list" />
...
<Disclaimer>
  Note: you can also follow along with this tutorial on GitHub. Visit the list of commits here:
  https://github.com/vnglst/phoenix_cursors/commits/main
</Disclaimer>
...
function cursorTemplate({ x, y, name, color }) {
  const li = document.createElement('li');
  li.classList = 'flex flex-col absolute pointer-events-none whitespace-nowrap overflow-hidden';
  li.style.left = x + 'px';
  li.style.top = y + 'px';
...
<Disclaimer>
  Many thanks to [Dmitri Don](https://twitter.com/DmitriDon) for giving this tutorial a try and reporting any mistakes
  and unclarities.
</Disclaimer>
