...
const OPTIMIZED_DIR = path.join(
  __dirname,
  '../public/static/photography-optimized',
)
...
async function generatePhotosData() {
      ...
      for (const width of WIDTHS) {
        ...
        if (await fileExists(path.join(imageDir, `${width}.jpg`))) {
          srcSetJpegParts.push(
            `/static/photography-optimized/${safeName}/${width}.jpg ${width}w`,
          )
        }
        if (await fileExists(path.join(imageDir, `${width}.webp`))) {
          srcSetWebpParts.push(
            `/static/photography-optimized/${safeName}/${width}.webp ${width}w`,
          )
        }
      }
      srcSetJpegParts.push(
        `/static/photography-optimized/${safeName}/original.jpg ${metadata.width || 0}w`,
      )
      srcSetWebpParts.push(
        `/static/photography-optimized/${safeName}/original.webp ${metadata.width || 0}w`,
      )
  ...
  console.log(
    `✅ Photo metadata written to ${path.relative(process.cwd(), OUTPUT_FILE)}`,
  )
